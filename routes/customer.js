...
router.get('/records/:id', async (req, res) => {
    try{
        ...
        if (!records) return res.status(404).send({'Error':'No se encuentran el radicado.'}); // Error 404 
        if (!records[0]) return res.status(404).send({'Error':'No se encuentran el radicado.'}); // Error 404 
